...
fun getChallenge(year: Int, day: Int, separator: String? = "\n") =
    (if (year == 0) "part3" else "inputs/$year/day$day").let {
        val text = when {
            ...
            Path("session.txt").exists() && day in 1..25 -> runBlocking {
                ...
                if (now < puzzleReleaseInstant) {
                    ...
                    delay(timeUntilRelease)
                }

                downloadPuzzle(year, day)
            }
            Path("session.txt").exists() && day in 31..55 -> runBlocking {
...
private val httpClient = HttpClient(httpEngine) {
    defaultRequest {
        header("Cookie", "session=${Path("session.txt").readText().trim()}")
    }
}

internal suspend fun downloadPuzzle(year: Int, day: Int): String {
 ...
 */
suspend fun attemptDownloadTest(year: Int, day: Int): String {
    val session = Path("session.txt").readText().trim()
    val url = "https://adventofcode.com/$year/day/$day"
    val response = httpClient.get(url)
    val result = response.bodyAsText()

    val element = Ksoup.parse(result).selectFirst("main article pre code")
