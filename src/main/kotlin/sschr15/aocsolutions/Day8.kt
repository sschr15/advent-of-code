...
fun day8() {
    ...
    input.sumOf { (input, output) ->
        fun thing(n: Int) = { it: String ->
            n in segmentCounts[it.length]
        }

        fun first(n: Int) = output.firstOrNull(thing(n)) ?: input.first(thing(n))
        fun allPossible(n: Int) = (output.filter(thing(n)) + input.filter(thing(n)))
        ...
        // segment 3 is the is in `3` and `4`, but not `7` (and that's all we know right now)
